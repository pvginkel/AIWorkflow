# Plan refinement — the interview, its readout, the replacement, and its first read

> *Written 2026-09-02 from session notes at plugin 0.9.13; every figure re-checked on 2026-09-03
> against the pushed `docs/research/plan-interview-2026-09-01.md`, and § What judged it added from
> the first five slices' read.*

This doc covers the interactive half of planning: how `/dev:plan-slice` pins a slice's requirements
with the operator today, what a read of 49 planning sessions found wrong with the way it asked, the
ruling that replaced the question dialogs with a per-slice refinement document, and what its first
five slices showed. The mechanics of the plan loop that follows the interview
...
**Measured.** On 2026-09-01 the operator asked for the dialogs to be read, with a brief
...
**Ruled.** No dialogs in `/dev:plan-slice`, ever. In their place:
...
into a confident option. Shipped as dev 0.9.19; the contract is `plugins/dev/docs/refinement.md`.
...
recommendation (86 of 317); the answer is a click, so "the operator's words" in the
...
is the argument as the sources give it; what the first five slices measured is below.

## What judged it (2026-09-03)

The pre-registered read ran on the first five slices planned on it — KubeCoder 198–202, all on
plugin 0.9.20, four of the five docs read cold the next morning
([`refinement-read-2026-09-03.md`](../research/refinement-read-2026-09-03.md)):

- **Dialogs: zero** in all five sessions (`plan_qa_readout.py table`), no relapse after a reframe,
  no dismissal. The declined deny-list backstop stays declined.
- **The operator's messages:** twelve decisions, no deviation from a recommendation, one comment in
  the file (a fact only the operator held), two questions back — one a reframe answered in prose,
  one a design change of the operator's own the doc had not offered — and two explicit
  delegations: "I trust you get this right", "I don't have the faintest about the other one".
- **Readable cold: no.** The docs were 27–35 KB; in three of five more than half was a
  premise-by-premise verification with functions quoted whole, and eight of the twelve decisions
  were engineering choices the session could have ranked itself. The operator: "a wall of text I
  can't make heads or tails of … I really don't believe it has value. It shouldn't be necessary",
  and "assume the low number of responses is to a large degree due to me not understanding the
  document."
- **The premise check earned its sub-agent** — 198's handover paraphrased a seam that had moved
  (six corrections), 202 had a fifth clone-host site and a decision record saying the opposite of
  the target — but its reader is the plan-writer, not the operator.

The design's premise, that the operator reads the recommendation's grounds and refutes them, is not
what happened; the grounds were skimmed and the recommendations trusted. Reworked as 0.9.21 on the
operator's four rulings: the doc opens with an introduction of one to three paragraphs, carries no
code and no handles (the "code in full" rule was a misread of the dialog brief — the code cut off
there was UI mock-ups in option text), puts a decision only where the operator could plausibly rule
the other way, and lists what the session settled in one sentence each, filtered to what touches a
