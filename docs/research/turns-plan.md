...
- [x] **T3a-kc** — **shipped 2026-08-23 (0.9.8)**: kc passes the flags through, `run_kc_session`
  ...
  **confirmed on 173/174/175** — every role −6.5 to −7 k against the corpus, no MCP call from a
  headless role ([t4-read-2026-08-23.md](t4-read-2026-08-23.md) § 4)
- [x] **T3b** frictions — reduced by T1 to W2 alone, **shipped 2026-08-23 (0.9.6)**; the hook
  programme is dead (the other fumbles are wrong-path guesses). First read: the writer's
...
**Read, preliminary (2026-08-23; 173/174/175 on 0.9.8, 179 on 0.9.7 — 16 writer sessions; more
slices before judging).** Full tables and the per-session lines in
[t4-read-2026-08-23.md](t4-read-2026-08-23.md), regenerated by `tools/t4_readout.py all`; the
comparators are the corpus's 2–4-phase band (the size match this section asked for) and the
16x–170 tail (the harness and effort match). **The mechanism is doing what it was built to do:**
whole-plan reads before the first edit 0 of 16 (corpus 182 of 184 sessions), every remaining
plan.md touch the done-record append; no verification.json / slice.md / CLAUDE.md reads; turn 1 is
`git branch` + the target file; orientation before the first edit median 8–10 against 11.5 (band)
and 16 (tail), context at the first edit 64 k against 77 k / 85 k. The one thing the digest sent a
writer back to the file for was the *next* phase's body (175 P3) — by design. **Cost:** writer
$/phase $2.82 against $4.83–5.53 — −40 % pooled, writer turns/phase −33 %; the median r1 writer
session, though, is only −8 % ($2.35 vs $2.56) — the pooled gain is the absent long tail (p90 47 vs
78 turns, max 50 vs 150), which ten sessions cannot attribute. Whole-slice $/phase −17 % against
the band, the rest of a slice being roles T4 does not touch. The digest measures ≈ 9–10 k tokens on
the real dispatch (writer `ctx1` 35.5 k vs the reviewer's 25.8 k in the same slices), above the
7.7 k in § Shipped — ≈ 3 chars/token, not 3.9; with T3 the net `ctx1` is +4 k on the corpus.
**Quality, all inside the baseline:** r1 blocking phases 3/13, every one a test-reach gap found by
reviewer mutation and none a plan-context miss; refuted 0, gate-red 0, abstentions 0; rework
4.9–17.7 %; 173's appended phase is a rebase conflict with the parallel lane 174. Nothing trips
the kill rule. **Confounds for the next read:** Claude Code 2.1.241 — the writers made zero
Read/Edit/Write tool calls in all 16 sessions (a shift that began at 2.1.234 and is in the corpus
tail; `first_edit_turn` is the orientation measure, not the Edit-tool one); the band's early
slices ran under other effort settings (compare thinking and output per turn against the tail
only). **Open:** whether the dollar effect is the median's −8 % or the pool's −40 % — 2–3 more
slices; the doc-writer is the most expensive single session in all four slices ($6.3–10.5,
`ctx_max` 168–249 k), T6's case as expected.
