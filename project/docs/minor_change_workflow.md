...
It runs the subproject's check/build/test commands, lists working-tree changes, and flags structural gaps (a schema change without a migration or test-data update, a source change with no matching test change). Resolve every command failure; resolve or justify every structural warning; review the change list for scope bleed. The structural check only confirms *some* test changed — confirm the new tests actually cover the changed behaviour.
