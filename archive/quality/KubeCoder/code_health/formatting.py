...
def _print_table(
    ...
    for i, r in enumerate(ranked, 1):
        ...
        if len(r.findings) > 3:
            ...
            more = _dim(f"- ... {len(r.findings) - 3} more findings (+{remaining:.1f})")
            print(f"         {more}")
        print()
...
def _print_rule_summary(reports: list[FileReport]) -> None:
    ...
    for rule in sorted(rule_points, key=lambda k: rule_points[k], reverse=True):
        label = _cyan(f"{rule:30s}")
        pts = _bold(f"{rule_points[rule]:8.1f}")
        print(f"    {label}  {rule_counts[rule]:4d} hits  {pts} pts")
    print()
